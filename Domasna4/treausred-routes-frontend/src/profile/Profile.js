...
const Profile = () => {
    ...
    useEffect(() => {
        const fetchUserEmail = async () => {
            try {
                const response = await fetch("https://graceful-yoke.railway.internal/authUser-service/user/profile", {
                    method: "GET",
                    headers: {
    ...
    const handleChangeEmail = async () => {
        try {
            const response = await fetch(`https://graceful-yoke.railway.internal/authUser-service/user/changemail/${newEmail}`, {
                method: "PUT",
                headers: {
                    'Authorization': `Bearer ${localStorage.getItem('jwtToken')}`
                },
