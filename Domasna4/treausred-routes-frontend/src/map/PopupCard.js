...
const PopupCard = ({obj, index, setAverageRatings, setRouteSites, handleAddToRoute}) => {
    ...
    const handleToggleFavorite = async (objectId, locationName) => {
        try {
            const response = await fetch('http://graceful-yoke.railway.internal/favorite-service/favorites', {
    ...
    const handleStarClick = async (objectId, rating) => {
        try {
            const response = await fetch(`http://graceful-yoke.railway.internal/review-service/reviews/${objectId}/${rating}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('jwtToken')}`
            ...
            if (response.ok) {
                ...
                const avgRatingResponse = await fetch(`http://graceful-yoke.railway.internal/review-service/reviews/rating/${objectId}`, {
                    headers: {
                        'Authorization': `Bearer ${localStorage.getItem('jwtToken')}`
                    }
                });
