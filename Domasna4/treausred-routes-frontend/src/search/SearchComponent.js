...
const SearchComponent = ({updateMarkers, focusTarget, focusMap}, initialCenter) => {
    ...
    useEffect(() => {
        fetch(`http://localhost:9000/element-service/api/search?query=${query}`, {
            headers: {
    ...
    const onSearch = async (searchTerm) => {
        await fetchData(`http://localhost:9000/element-service/api/search?query=${searchTerm}`, setObjsForFocus)
        var coordinates = calculateNewCenter(objsForFocus, initialCenter)
        if(coordinates.length === 1){
            updateMarkers(`http://localhost:9000/element-service/api/search?query=${searchTerm}`)
